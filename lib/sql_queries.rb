...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS NEW_BAL FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING projects.funding_goal <= NEW_BAL;"
end
